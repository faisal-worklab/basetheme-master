...
namespace DivsFlow\Boilerplate;
...
class WP_Theme_Boilerplate {
	...
	public static function block( $block, $callback_function ) {
		list( $dfb_block_id, $dfb_block_fields,$dfb_option_fields ) = DivsFlow::defaults( $block['id'] );

		// Set the block name for it's ID & class from it's file name.
		$dfb_block_name   = $block['name'];
		$dfb_block_name   = str_replace( 'acf/', '', $dfb_block_name );
		$dfb_block_styles = DivsFlow::convert_to_css( $block );
		// Set the preview thumbnail for this block for gutenberg editor view.
		if ( isset( $block['data']['preview'] ) && 'preview' === $block['mode'] && isset( $block['data']['preview'] ) ) {
			$version = filemtime( get_template_directory() . '/blocks/' . $dfb_block_name . '/' . $block['data']['preview'] );
			echo '<img src="' . esc_url( get_template_directory_uri() . '/blocks/' . $dfb_block_name . '/' . $block['data']['preview'] ) . '?v=' . esc_html( $version ) . '" style="width:100%; height:auto;">';
		}

		// create align class ("alignwide") from block setting ("wide").
		$dfb_var_align_class = $block['align'] ? 'align' . $block['align'] : '';

		// Get the class name for the block to be used for it.
		$dfb_var_class_name = ( isset( $block['className'] ) ) ? $block['className'] : null;

		// Making the unique ID for the block.
		$dfb_block_html_id = 'block-' . $dfb_block_name . '-' . $block['id'];

		// Making the unique ID for the block.
		if ( $block['name'] ) {
			$dfb_block_name = $block['name'];
			$dfb_block_name = str_replace( '/', '-', $dfb_block_name );
			$dfb_var_name   = 'block-' . $dfb_block_name;
		}
		echo '<div id="' . esc_html( $dfb_block_html_id ) . '" class="' . esc_html( $dfb_var_align_class . ' ' . $dfb_var_class_name . ' ' . $dfb_var_name ) . ' block-' . esc_html( $dfb_block_name ) . '" style="' . esc_html( $dfb_block_styles ) . '"> ';

		$spacers = self::get_spacer( $dfb_block_fields );
		DivsFlow::the_spacer( $spacers, 'top' );
		$callback_function( $dfb_block_id, $dfb_block_name, $dfb_block_fields, $dfb_option_fields );
		DivsFlow::the_spacer( $spacers, 'bottom' );
		echo '</div>';
		wp_enqueue_script( 'wp-theme-' . str_replace( 'acf-', '', $dfb_block_name ) );
	}
	/**
	 * Query Boilerplate.
	 *
		...
		if ( $post_type ) {
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					$query->the_post();
					$dfb_var_post_id = get_the_ID();

					list( $dfb_var_post_id, $dfb_fields, $dfb_option_fields ) = DivsFlow::defaults( $dfb_var_post_id );
					// Include specific template for the content.
					if ( isset( $template ) ) {
						get_template_part( 'partials/content', $template );
					} else {
						$render_fun( $dfb_var_post_id, $dfb_fields, $dfb_option_fields );
					}
				}
			} else {
				// If no content, include the "No posts found" template.
			...
			if ( $query->have_posts() && $query->max_num_pages > 1 && $pagination ) {
				...
				DivsFlow::$pagination_fun( $query->max_num_pages );
				echo '</div>';
			}
			return $query;
		}
	 ...
	 * Helper Function
	 *
	 * @param mixed $dfb_block_fields block fields data.
	 *
	 * @return array
	 */
	private static function get_spacer( $dfb_block_fields ) {
		if ( $dfb_block_fields ) {
			foreach ( $dfb_block_fields as $dfb_block_field ) {
				if ( is_array( $dfb_block_field ) ) {
					foreach ( $dfb_block_field as $dfb_key => $dfb_block_sub_field ) {
						if ( 'top_spacer' === $dfb_key || 'bottom_spacer' === $dfb_key ) {
							return $dfb_block_field;
						}
					}
				}
			}
...
class_alias( 'DivsFlow\Boilerplate\WP_Theme_Boilerplate', 'Boilerplate' );
